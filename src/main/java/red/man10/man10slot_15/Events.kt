...
class Events(val plugin: Man10Slot_15): Listener {
    ...
    fun onClick(e: PlayerInteractEvent) {
        ...
        if (plugin.leverloc.containsKey(b!!.location)) {
            ...
            v.withdraw(p.uniqueId, slot.price)
            ...
            object : BukkitRunnable() {
                ...
                override fun run() {
                    if (plugin.signloc.containsKey(key)) {

                        val s = plugin.signloc[key]!!.world!!.getBlockAt(plugin.signloc[key]!!)

                        if (s.state is Sign) {

                            val a = s.state as Sign
    ...
    @EventHandler
    fun signPlace(e: SignChangeEvent) {
        ...
        e.setLine(0, "§9§l=======================================")
        e.setLine(1, "§l[${slot.slot_name}§r§l]")
        e.setLine(2, "§lcount:§a§l${slot.spincount}")
        e.setLine(3, "§9§l=======================================")

        plugin.signloc[key] = b.location

        p.sendMessage("${plugin.prefix}§a設定が完了しました")
    ...
    fun onDrop(e: ItemSpawnEvent) {
        ...
        if (!i.itemStack.itemMeta!!.displayName.contains("§6MSlotCoin")) return
